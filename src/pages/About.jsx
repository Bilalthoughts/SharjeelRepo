import React from "react";
import SimpleSlider from "../components/Reviews";

function About() {
    return (
        <>
            {/* =========================== 3. Page Header Section =========================================== */}
            <div className="page-header">
                <div className="about-banner-shape-left wow animate__animated animate__fadeInLeft">
                    <img
                        src="assets/images/shapes/03_shape-01.png"
                        className="wow animate__animated animate__pulse animate__infinite"
                        alt="habu"
                    />
                </div>
                <div className="about-banner-shape-right wow animate__animated animate__fadeInRight animate__delay-1s">
                    <img
                        src="assets/images/shapes/03_shape-02.png"
                        className="wow animate__animated animate__pulse animate__infinite"
                        alt="habu"
                    />
                </div>
                <div className="container">
                    <div className="page-header-text wow animate__animated animate__fadeInDown">
                        <h1 className="page-title">About</h1>
                        <span>Home</span>
                        <span className="span-divider font-weight-bold">|</span>
                        <span className="font-weight-bold">About</span>
                        <div className="page-banner-shape-title">
                            <img
                                src="assets/images/shapes/03_shape-03.png"
                                className="wow animate__animated animate__fadeInUp animate__delay-1s"
                                alt=""
                            />
                        </div>
                    </div>
                </div>
            </div>
            {/* =========================== 4. About Section =========================================== */}
            <section className="speciality-area">
                <div className="container">
                    <div className="row justify-content-between">
                        <div className="col-xl-5 col-lg-6 about-area-image wow animate__animated animate__fadeInLeft animate__delay-1s">
                            <img src="assets/images/banner/home-banner-4.png" alt="Habu" />
                        </div>
                        <div className="col-lg-6 speciality-text wow animate__animated animate__fadeInRight animate__delay-1s">
                            <div className="section-heading w-100 mb-28 mt-0">
                                <h2>Who we are?</h2>
                                <p>
                                    We have a group of very expert people in different sectors. e.g-
                                    Graphic Design, Web Design, Web Development, Digital Marketing,
                                    Social Media Marketing, E-mail Marketing etc.
                                </p>
                            </div>
                            <div className="speciality">
                                <div className="sp-text">
                                    <svg
                                        data-name="Why Us"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                        width={24}
                                        height={25}
                                        viewBox="0 0 24 25"
                                    >
                                        <g>
                                            <image
                                                width={24}
                                                height={25}
                                                xlinkHref="data:img/png;base64,"
                                            />
                                        </g>
                                    </svg>
                                    <p>
                                        Lorem ipsum, dolor sit amet consectetur adipisicing adipisicing
                                        elit.
                                    </p>
                                </div>
                            </div>
                            <div className="speciality">
                                <div className="sp-text">
                                    <svg
                                        data-name="Why Us"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                        width={24}
                                        height={25}
                                        viewBox="0 0 24 25"
                                    >
                                        <g>
                                            <image
                                                width={24}
                                                height={25}
                                                xlinkHref="data:img/png;base64,"
                                            />
                                        </g>
                                    </svg>
                                    <p>Lorem ipsum, dolor sit amet consectetur adipisicing.</p>
                                </div>
                            </div>
                            <div className="speciality">
                                <div className="sp-text">
                                    <svg
                                        data-name="Why Us"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                        width={24}
                                        height={25}
                                        viewBox="0 0 24 25"
                                    >
                                        <g>
                                            <image
                                                width={24}
                                                height={25}
                                                xlinkHref="data:img/png;base64,"
                                            />
                                        </g>
                                    </svg>
                                    <p>
                                        Lorem ipsum, dolor sit amet consectetur adipisicing consectetur.
                                    </p>
                                </div>
                            </div>
                            <div className="speciality">
                                <div className="sp-text">
                                    <svg
                                        data-name="Why Us"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                        width={24}
                                        height={25}
                                        viewBox="0 0 24 25"
                                    >
                                        <g>
                                            <image
                                                width={24}
                                                height={25}
                                                xlinkHref="data:img/png;base64,"
                                            />
                                        </g>
                                    </svg>
                                    <p>Lorem ipsum, dolor sit amet consectetur adipisicing.</p>
                                </div>
                            </div>
                            <div className="speciality">
                                <div className="sp-text">
                                    <svg
                                        data-name="Why Us"
                                        xmlns="http://www.w3.org/2000/svg"
                                        xmlnsXlink="http://www.w3.org/1999/xlink"
                                        width={24}
                                        height={25}
                                        viewBox="0 0 24 25"
                                    >
                                        <g>
                                            <image
                                                width={24}
                                                height={25}
                                                xlinkHref="data:img/png;base64,"
                                            />
                                        </g>
                                    </svg>
                                    <p>
                                        Lorem ipsum, dolor sit amet consectetur adipisicing dolor sit
                                        amet.
                                    </p>
                                </div>
                            </div>
                            <a
                                href="contact.php"
                                className="wow animate__animated animate__flipInX large-blue-button mt-40 speciality-button"
                            >
                                Contact Us
                            </a>
                        </div>
                    </div>
                </div>
            </section>
            {/* =========================== 5. Companies Section =========================================== */}
            <div className="companies d-flex align-items-center">
                <div className="container">
                    <div className="row justify-content-between companies-logo">
                        <img
                            src="assets/images/logo/company-logo/logo-kupa.png"
                            className="my-auto"
                            alt="Habu"
                        />
                        <img
                            src="assets/images/logo/company-logo/logo-tarabara.png"
                            className="my-auto"
                            alt="Habu"
                        />
                        <img
                            src="assets/images/logo/company-logo/logo-shiram.png"
                            className="my-auto"
                            alt="Habu"
                        />
                        <img
                            src="assets/images/logo/company-logo/logo-codex.png"
                            className="my-auto"
                            alt="Habu"
                        />
                        <img
                            src="assets/images/logo/company-logo/logo-kupa.png"
                            className="my-auto"
                            alt="Habu"
                        />
                        <img
                            src="assets/images/logo/company-logo/logo-tarabara.png"
                            className="my-auto"
                            alt="Habu"
                        />
                        <img
                            src="assets/images/logo/company-logo/logo-shiram.png"
                            className="my-auto"
                            alt="Habu"
                        />
                        <img
                            src="assets/images/logo/company-logo/logo-codex.png"
                            className="my-auto"
                            alt="Habu"
                        />
                    </div>
                </div>
            </div>
            {/* =========================== 6. Our Team Section =========================================== */}
            <section className="our-team">
                <div className="container">
                    <div className="section-heading text-center mx-auto">
                        <h2>Our Amazing Team</h2>
                        <p>
                            We have people of multiple kind in the house. Together we can provide
                            high quality work to satisfy you.
                        </p>
                    </div>
                    <div className="team-members wow animate__animated animate__fadeInUp">
                        <div className="row">
                            <div className="col-xl-3 col-lg-3 col-sm-6">
                                <div className="member-bio">
                                    <img src="assets/images/team-members/03-team-member-01.png" alt="Habu" />
                                    <h2>Jackson Nash</h2>
                                    <h4>Programmer</h4>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-facebook-f" />
                                        </a>
                                    </div>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-twitter" />
                                        </a>
                                    </div>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-instagram" />
                                        </a>
                                    </div>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-youtube" />
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-sm-6">
                                <div className="member-bio">
                                    <div className="member-avatar">
                                        <img
                                            src="assets/images/team-members/03-team-member-02.png"
                                            alt="Habu"
                                        />
                                    </div>
                                    <h2>John Harper</h2>
                                    <h4>Web Developer</h4>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-facebook-f" />
                                        </a>
                                    </div>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-twitter" />
                                        </a>
                                    </div>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-instagram" />
                                        </a>
                                    </div>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-youtube" />
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-sm-6">
                                <div className="member-bio">
                                    <img src="assets/images/team-members/03-team-member-03.png" alt="Habu" />
                                    <h2>Jason Money</h2>
                                    <h4>Digital Marketer</h4>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-facebook-f" />
                                        </a>
                                    </div>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-twitter" />
                                        </a>
                                    </div>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-instagram" />
                                        </a>
                                    </div>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-youtube" />
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <div className="col-xl-3 col-lg-3 col-sm-6">
                                <div className="member-bio">
                                    <img src="assets/images/team-members/03-team-member-04.png" alt="Habu" />
                                    <h2>Lady Cash</h2>
                                    <h4>Graphics Designer</h4>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-facebook-f" />
                                        </a>
                                    </div>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-twitter" />
                                        </a>
                                    </div>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-instagram" />
                                        </a>
                                    </div>
                                    <div className="member-social-profile">
                                        <a className="mx-auto" href="#">
                                            <i className="fab fa-youtube" />
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
            {/* =========================== 7. Experience Section =========================================== */}
            <div className="experience d-flex align-items-center">
                <div className="container">
                    <div className="row justify-content-between">
                        <div className="speciality-box col-lg-3 col-sm-6">
                            <div className="sp-box-icon text-center">
                                <svg
                                    data-name="Why Us"
                                    xmlns="http://www.w3.org/2000/svg"
                                    xmlnsXlink="http://www.w3.org/1999/xlink"
                                    width={53}
                                    height={53}
                                    viewBox="0 0 53 53"
                                >
                                    <g>
                                        <g data-name={1}>
                                            <image
                                                width={53}
                                                height={53}
                                                xlinkHref="data:img/png;base64,"
                                            />
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div className="sp-box-text text-center">
                                <h3>
                                    <span className="counter">3200</span>+
                                </h3>
                                <p>Satisfied Customers</p>
                            </div>
                        </div>
                        <div className="speciality-box col-lg-3 col-sm-6">
                            <div className="sp-box-icon text-center">
                                <svg
                                    data-name="Why Us"
                                    xmlns="http://www.w3.org/2000/svg"
                                    xmlnsXlink="http://www.w3.org/1999/xlink"
                                    width={55}
                                    height={49}
                                    viewBox="0 0 55 49"
                                >
                                    <g>
                                        <g data-name={2}>
                                            <image
                                                id="calendar"
                                                width={55}
                                                height={49}
                                                xlinkHref="data:img/png;base64,"
                                            />
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div className="sp-box-text text-center">
                                <h3>
                                    <span className="counter">03</span>+
                                </h3>
                                <p>Years Of Experience</p>
                            </div>
                        </div>
                        <div className="speciality-box col-lg-3 col-sm-6">
                            <div className="sp-box-icon text-center">
                                <svg
                                    data-name="Why Us"
                                    xmlns="http://www.w3.org/2000/svg"
                                    xmlnsXlink="http://www.w3.org/1999/xlink"
                                    width={55}
                                    height={55}
                                    viewBox="0 0 55 55"
                                >
                                    <g>
                                        <g data-name={3}>
                                            <image
                                                id="verified"
                                                width={55}
                                                height={55}
                                                xlinkHref="data:img/png;base64,"
                                            />
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div className="sp-box-text text-center">
                                <h3>
                                    <span className="counter">1200</span>+
                                </h3>
                                <p>Projects Completed</p>
                            </div>
                        </div>
                        <div className="speciality-box col-lg-3 col-sm-6">
                            <div className="sp-box-icon text-center">
                                <svg
                                    data-name="Why Us"
                                    xmlns="http://www.w3.org/2000/svg"
                                    xmlnsXlink="http://www.w3.org/1999/xlink"
                                    width={55}
                                    height={55}
                                    viewBox="0 0 55 55"
                                >
                                    <g>
                                        <g data-name={4}>
                                            <image
                                                id="speed"
                                                width={55}
                                                height={55}
                                                xlinkHref="data:img/png;base64,"
                                            />
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div className="sp-box-text text-center">
                                <h3>
                                    <span className="counter">900</span>+
                                </h3>
                                <p>Websites Launched</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            {/* =========================== 8. Review Section =========================================== */}
            <SimpleSlider />
        </>


    );
}

export default About;